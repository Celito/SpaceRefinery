...
public class OldTube : OldStructure
{
    ...
    private List<ConnectionPoint> _extensionTips;
    ...
    private List<ConnectionPoint> _currPossibleConnections;
    ...
    override protected void VirtualAwake()
    {
        ...
        _currPossibleConnections = new List<ConnectionPoint>();
        _sections.Add(
            new SectionInfo
            {
                length = 1,
    ...
    public void ExtendTube(GridManager.Direction dir)
    {
        ...
        for (int dirIndex = 1; dirIndex < 7; dirIndex++)
        {
            ...
            else if (extensionTip == null && !hitSomething)
            {
                ...
                tipScript.SetTipType(ConnectionPoint.ConnectionPointType.Extension);
            ...
            if (hitSomething && hitInfo.collider.gameObject.tag == "TubeTip")
            {
                ...
                ConnectionPoint tubesTip = hitObject.GetComponent<ConnectionPoint>();
                var reverseCurDirection = GridManager.instance.OppositeDir(curDirection);
                tubesTip.AddPossibleConnection(new KeyValuePair<GridManager.Direction, OldTube>(reverseCurDirection, this));
    ...
    public void CreateExtensionTips(ConnectionPoint.ConnectionPointIOType tipIOType)
    {
        _extensionTips = new List<ConnectionPoint>();
        for (int dirIndex = 1; dirIndex < 7; dirIndex++)
        {
            ...
            if (hitSomething && hitInfo.collider.gameObject.tag == "TubeTip")
            {
                ...
                tubesTip.AddPossibleConnection(new KeyValuePair<GridManager.Direction, OldTube>(reverseDirection, this));
                _currPossibleConnections.Add(tubesTip);
            }
        }
    }
